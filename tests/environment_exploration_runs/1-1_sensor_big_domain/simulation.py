...
class Environment:
    ...
    def _track_obstacles(self, seed: int = None) -> list[Obstacle]:
        ...
        segment_size = path_span/self.obstacle_count

        xs = np.linspace(x_car, x_target, self.obstacle_count+1)
        noise = np.hstack([np.zeros(1), rng.normal(loc=0, scale=segment_size/10, size=self.obstacle_count-1), np.zeros(1)]) # <- no noise at the ends
        xs = xs + noise
        ys = np.linspace(y_car, y_target, self.obstacle_count+1)
        noise = np.hstack([np.zeros(1), rng.normal(loc=0, scale=path_span/20, size=self.obstacle_count-1), np.zeros(1)]) # <- no noise at the ends
        ys = ys + noise

        xs_m = []
        ys_m = []

        for i in range(0, self.obstacle_count-1):
